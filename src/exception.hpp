...
static const int max_logsize_stackdump =
        32 + 80 + (stackdump_line_count * 32) + (10 * stackdump_words_per_line * stackdump_line_count);
static const int max_logsize_backtrace = 32 + max_backtrace_ever * (MAX_PATH + symbol_max + 90);
static const int max_logsize_ever =
        32 + max_logsize_basic + max_logsize_regs + max_logsize_stackdump + max_logsize_backtrace;
...
static int LogException(char *buffer, size_t max, LPEXCEPTION_POINTERS pException, bool bLogRegisters, bool bLogStack,
                        bool bLogBacktrace);
...
ExceptionTracer::ExceptionTracer(char *buffer, size_t max, LPEXCEPTION_POINTERS pException) :
        buffer(buffer), exception(*pException), record(*pException->ExceptionRecord),
        context(*pException->ContextRecord) {
    this->buffer = buffer;
...
void ExceptionTracer::PrintRegisters() {
    ...
    auto PrintFloatRegister = [this, &regs_in_line](const char *reg_name, int reg_num, uint32_t reg_value1,
                                                    uint32_t reg_value2, uint32_t reg_value3, uint32_t reg_value4) {
        Print("%s%02d: 0x%08X 0x%08X 0x%08X 0x%08X  [ %f %f %f %f ]", reg_name, reg_num, reg_value1, reg_value2,
              reg_value3, reg_value4,
...
void ExceptionTracer::PrintStackdump() {
    ...
    auto GetStackBottom = [&mbi](uintptr_t base) {
        VirtualQuery((void *) base, &mbi,
                     sizeof(mbi));                               // Find uncommited region of the stack
        VirtualQuery((char *) mbi.BaseAddress + mbi.RegionSize, &mbi, sizeof(mbi));   // Find guard page
        VirtualQuery((char *) mbi.BaseAddress + mbi.RegionSize, &mbi,
                     sizeof(mbi));   // Find commited region of the stack
        auto last = (uintptr_t) mbi.BaseAddress;
        return (base + (last - base) +
                mbi.RegionSize);                             // base + distanceToLastRegion + lastRegionSize
    };
...
void ExceptionTracer::PrintBacktrace() {
    ...
    if (SymInitialize(GetCurrentProcess(), 0, TRUE)) {
        has_symbol_api = true;
        old_options = SymSetOptions(
                SYMOPT_DEFERRED_LOADS | SYMOPT_LOAD_LINES | SYMOPT_NO_PROMPTS | SYMOPT_FAIL_CRITICAL_ERRORS);
    }
    ...
    EnterScope();
    {
        ...
        while (auto trace = tracer.Walk()) {
            ...
            if (has_symbol_api)
                has_sym = trace->pc ? !!SymFromAddr(GetCurrentProcess(), (DWORD64) trace->pc, &displacement, &symbol)
                                    : false;

            // Print everything up, this.... Ew, this looks awful!
            Print(backtrace_count == 1 ? "=>"
                                       : "  ");                          // First line should have '=>' to specify where it crashed
...
static const char *GetExceptionCodeString(unsigned int code) {
    switch (code) {
        ...
        case EXCEPTION_DATATYPE_MISALIGNMENT:
            return "Data type misalignment exception";
        case EXCEPTION_FLT_DENORMAL_OPERAND:
            return "Denormal float operand";
